...
```{r burbujas-costos-corregido-final, fig.width=5, fig.height=8, cache=FALSE}
# Cargar librerías
library(ggplot2)
library(dplyr)
library(plotly)
...
```{r costos-materiales-final-sin-herramientas, fig.width=9, fig.height=4, cache=FALSE}
# Cargar librerías
library(ggplot2)
library(dplyr)
library(tidyr)
...
ggplotly(p_columnas, tooltip = "text") %>%
  layout(
    ...
    yaxis = list(title = "Incidencia Costo Obra (%)"),
    showlegend = TRUE
  )
```
